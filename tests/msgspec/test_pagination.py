...
def test_paginator_with_sort(example_dataset):
    ...
    # Paginate all orcs
    paginator = Paginator(Orc, filter_kwargs={"sort": [("name", DESC)]})
...
def test_paginator_with_projection(example_dataset):
    ...
    # Paginate all orcs
    paginator = Paginator(Orc, filter_kwargs={"projection": {"name": False}})
...
def test_page(example_dataset):
    ...
    assert page.get_offset(orc) == 992
